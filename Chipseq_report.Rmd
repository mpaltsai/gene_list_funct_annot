...
ChIPseq = read.csv("ChIPseq_genes.csv", sep=",", header = F,
                     check.names=FALSE, skip=2, col.names=header0)
dim_new_ChIPseq <- dim(ChIPseq)
cat("dimensions of New_Targets file:") 
dim_ChIPseq

#upper case to genes strings
#ChIPseq <- apply(ChIPseq, 2, toupper)
#ChIPseq <- as.data.frame(ChIPseq)
...
count_genes_targets_genes_batch = fun(ChIPseq, genes_batch)
count_genes_targets_genes_OldProt = fun(ChIPseq, genes_OldProt)
count_genes_targets_genes_OldVSall = fun(ChIPseq, genes_oldVSall)
...
common_genes_targets_genes_batch <- fun2(ChIPseq, genes_batch)
common_genes_targets_genes_OldProt = fun2(ChIPseq, genes_OldProt)
common_genes_targets_genes_OldVSall = fun2(ChIPseq, genes_oldVSall)
...
fishers_targets_genes_batch <- my_fishers_test(ChIPseq, genes_batch)
fishers_targets_genes_OldProt = my_fishers_test(ChIPseq, genes_OldProt)
fishers_targets_genes_OldVSall = my_fishers_test(ChIPseq, genes_oldVSall)
```
...
write.table(fun_annot_mat_mm, "funct_annot_sign_fdr_genes_mmusculus.csv", sep=",",  na="")
